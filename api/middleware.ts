...
export default clerkMiddleware(async (auth, request: NextRequest) => {
  ...
  if (
    ...
    pathname.startsWith("/api/progress/addlanguage") ||
    pathname.startsWith("/")
  ) {
  ...
  // Generate CSRF token for page requests (to set cookie)
  // if (!pathname.startsWith("/api/") && !pathname.startsWith("/_next/")) {
  //   const { response } = generateAndSetCSRFToken();
  //   return response;
  // }

  // Auth API: NOTrequire auth token
  if (
