...
export async function POST(req: Request) {
  ...
  try {
    if (eventType === "user.created" || eventType === "user.updated") {
      ...
      await syncUserWithClerk(evt.data as ClerkUser);
    } else if (eventType === "session.created") {
      const { user_id: clerkId } = evt.data;

      // Now you can safely access session properties
      const ip = req.headers.get("x-forwarded-for") || "unknown";
      const userAgent = req.headers.get("user-agent") || "unknown";

      const parser = Bowser.getParser(userAgent); // works with any string
      ...
      // Optional: record a logout activity in your database
    ...
    //   // Extract the subscription from private metadata
    //   const subscription = data.private_metadata?.subscription;

    //   if (userId && subscription) {
    //     // Mirror it to public metadata for UI access
